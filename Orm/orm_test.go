...
func TestDBTransaction(t *testing.T) {
	fmt.Println("=================================Tranaction================================")
	data, err := DB().Table("goods").Where("goods_name", "LIKE", "%当家").Get()
	if err != nil {
	...
	update["goods_name"] = "大土匪头子"
	update["goods_stock"] = 1
	update["goods_status"] = 1
	afn, err := tx.Table("goods").Where("goods_name", "土匪头子").Update(update)
	if err != nil {
		t.Errorf("Update error %s", err)
	}
	fmt.Println("Update ", afn)
	afn, err = tx.Table("goods").Where("goods_name", "小当家").Delete()
	if err != nil {
		t.Errorf("Delete error %s", err)
	}
	fmt.Println("Delete ", afn)
	err = tx.DbRollBack()
	if err != nil {
