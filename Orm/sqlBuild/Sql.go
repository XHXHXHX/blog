...
func (this *sqlInfo) setJoin(buildInfo *SqlBuild, joinType, thisRelationField, relationCondition, thatRelationField string) {
	joinData := &joinInfo{
		buildInfo: buildInfo,
		joinType: joinType,
...
func (this *sqlInfo) InWhere(data *whereInfo) string {
	...
	for _, item := range value {
		this.preproParam = append(this.preproParam, string(item))
	}
...
func (this *sqlInfo) LittleChangeForWhere(boolean bool, subQuery *SqlBuild, s string) string {
	c := "AND "
	if boolean {
		c = "OR "
	}
